...
class Header extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      accountLink: commerce.customer.isLoggedIn() ? '/account' : '/login',
    });
  }
  ...
  render() {
    const { showMobileMenu, showCart, accountLink } = this.state;
    const { transparent, cart, t } = this.props;
    ...
    return (
      <header className="position-fixed top-0 left-0 right-0 font-weight-semibold no-print">
        <Cart isOpen={showCart} toggle={value => this.toggleCart()} />
        <div
          ref={this.header}
        ...
        >
          <div className="d-none d-sm-flex">
            <Link href="/collection">
              <a className="mr-4 font-color-black">{t('routes.shop')}</a>
            </Link>
            <Link href="/about">
              <a className="font-color-black">{t('routes.about')}</a>
            </Link>
          </div>
          <div className="logo-container">
            <svg 
              className="fill-black w-24 h-24 d-block d-sm-none cursor-pointer"
              onClick={this.toggleMobileMenu}
            >
              <title>Toggle menu</title>
              <use xlinkHref={showMobileMenu ? '/icons.svg#cross' : '/icons.svg#menu'}></use>
            </svg>
            <Link href="/">
              <a className="ml-2">
                <svg className="fill-brand700 h40 logo cursor-pointer">
                  <title>Site logo with text</title>
                  <use xlinkHref="/icons.svg#massi105-logo-text"></use>
                </svg>
              </a>
            </Link>
          </div>
          <div className="d-flex">
            <Link href={accountLink}>
              <a className="font-color-black mx-2 account">
